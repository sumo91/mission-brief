...
EXPECTED_RUNTIME_FILES = ["SKILL.md", "agents/openai.yaml", "references/mission-zero.md"]
...
def verify_release_runs(
    ...
    if set(baseline_cases) != {"b-30000001"}:
        raise VerificationError("baseline run does not contain the historical regression case")
    if baseline_cases["b-30000001"].get("verdict") not in {"PASSED", "FAILED", "INCONCLUSIVE"}:
        raise VerificationError("baseline run has no valid observation verdict")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if all(provided):
        ...
        result["evidence_aggregates"] = verify_release_runs(
            args.candidate_run,
            args.blind_run,
            args.baseline_run,
            candidate_digest,
